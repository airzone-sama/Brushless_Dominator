// Comment this out to revert to the Servo library.
// Be aware that the Chrony will not be as accurate
...
// Pin Definitions
#define PIN_LED PC13
#define PIN_MOTOR_A PB1
#define PIN_MOTOR_B PB0
#define PIN_MOTOR_C PA7
#define PIN_ESC_FET PA15
#define PIN_TRIGGER_FULL PB3
#define PIN_TRIGGER_HALF PB4
#define PIN_TRIGGER_REV PB5
#define PIN_ALT_MODE PB8
#define PIN_E_STOP PB9
#define PIN_PUSHER_FET PA6
#define PIN_PROFILE_A PB15
#define PIN_PROFILE_B PB12
#define PIN_SELECT_FIRE_A PB13
#define PIN_SELECT_FIRE_B PB14
#define PIN_CONFIG_MODE PA8
#define PIN_ENABLE_BT PB11
#define PIN_ENABLE_DEBUG PB10
#define PIN_MAG_SENSOR PA5
#define PIN_DART_IN_MAG_SENSOR PA4
#define PIN_PUSHER_RETURN_SENSOR PA3
#define PIN_CHRONO_A PA2
#define PIN_CHRONO_B PA1
#define PIN_BATTERY_MONITOR PA0
...
void setup() {
  ...
  DebugPrintln( F("Loading EEPROM") );
  LoadEEPROM();
  DebugPrintln( F("EEPROM Loaded, Selecting Battery Type") );
...
/*
 * Loads / manages the configuration
 */
void LoadEEPROM()
{
  ...
  Profiles[0].FullPower = EEPROM.read( 0x00 );
  Profiles[0].HalfPower = EEPROM.read( 0x02 );
  Profiles[0].ROF = EEPROM.read( 0x04 );
  Profiles[0].BurstSize = EEPROM.read( 0x06 );
  Profiles[0].MagSize = EEPROM.read( 0x08 );
  Profiles[0].FireOnEmptyMag = EEPROM.read( 0x0A );
  Profiles[0].AltAction = EEPROM.read( 0x0C );
    
  Profiles[1].FullPower = EEPROM.read( 0x10 );
  Profiles[1].HalfPower = EEPROM.read( 0x12 );
  Profiles[1].ROF = EEPROM.read( 0x14 );
  Profiles[1].BurstSize = EEPROM.read( 0x16 );
  Profiles[1].MagSize = EEPROM.read( 0x18 );
  Profiles[1].FireOnEmptyMag = EEPROM.read( 0x1A );
  Profiles[1].AltAction = EEPROM.read( 0x1C );

  Profiles[2].FullPower = EEPROM.read( 0x20 );
  Profiles[2].HalfPower = EEPROM.read( 0x22 );
  Profiles[2].ROF = EEPROM.read( 0x24 );
  Profiles[2].BurstSize = EEPROM.read( 0x26 );
  Profiles[2].MagSize = EEPROM.read( 0x28 );
  Profiles[2].FireOnEmptyMag = EEPROM.read( 0x2A );
  Profiles[2].AltAction = EEPROM.read( 0x2C );

  Profiles[3].FullPower = EEPROM.read( 0x30 );
  Profiles[3].HalfPower = EEPROM.read( 0x32 );
  Profiles[3].ROF = EEPROM.read( 0x34 );
  Profiles[3].BurstSize = EEPROM.read( 0x36 );
  Profiles[3].MagSize = EEPROM.read( 0x38 );
  Profiles[3].FireOnEmptyMag = EEPROM.read( 0x3A );
  Profiles[3].AltAction = EEPROM.read( 0x3C );        

  EStopOnPusherJam = EEPROM.read( 0x40 );
  EStopOnDartJam = EEPROM.read( 0x42 );
  CurrentProfile = EEPROM.read( 0x44 );

  // 32 bit value
  uint16 LDF1 = EEPROM.read( 0x46 ); 
  uint16 LDF2 = EEPROM.read( 0x48 ); 
  LifetimeDartsFired = LDF2;
  LifetimeDartsFired = LifetimeDartsFired << 16;
  LifetimeDartsFired += LDF1;

  // Print to debugging screen
  PrintProfile( 0 );
  PrintProfile( 1 );
  PrintProfile( 2 );
  PrintProfile( 3 );
  DebugPrintln( "\nSystem Values" );
  DebugPrint( "EStopOnPusherJam = " );
  DebugPrintln( EStopOnPusherJam );
  DebugPrint( "EStopOnDartJam = " );
  DebugPrintln( EStopOnDartJam );
  DebugPrint( "CurrentProfile = " );
  DebugPrintln( CurrentProfile );
  DebugPrint( "LifetimeDartsFired = " );
  DebugPrintULln( LifetimeDartsFired );

  if( !ValidateProfile( 0 ) ) CorruptData = true;
  if( !ValidateProfile( 1 ) ) CorruptData = true;
  if( !ValidateProfile( 2 ) ) CorruptData = true;
  if( !ValidateProfile( 3 ) ) CorruptData = true;
  if( (CurrentProfile < 0) || (CurrentProfile > 3) ) CorruptData = true;
  ...
  {
    DebugPrintln( F("Something wrong with EEPROM or held trigger while booting") );
    DebugPrintln( CorruptData );
    DebugPrintln( TriggerStatus == LOW );

    DefaultProfile( 0 );
    DefaultProfile( 1 );
    DefaultProfile( 2 );
    DefaultProfile( 3 );
    EStopOnPusherJam = true;
    EStopOnDartJam = true;
    CurrentProfile = 0;
    LifetimeDartsFired = 0;

    SaveEEPROM( true ); // Save the default set
  }
} 
...
void SaveEEPROM( bool FullSave )
{
  if( FullSave )
  {
    EEPROM.update( 0x00, Profiles[0].FullPower );
    EEPROM.update( 0x02, Profiles[0].HalfPower );
    EEPROM.update( 0x04, Profiles[0].ROF );
    EEPROM.update( 0x06, Profiles[0].BurstSize );
    EEPROM.update( 0x08, Profiles[0].MagSize );
    EEPROM.update( 0x0A, Profiles[0].FireOnEmptyMag );
    EEPROM.update( 0x0C, Profiles[0].AltAction );
    
    EEPROM.update( 0x10, Profiles[1].FullPower );
    EEPROM.update( 0x12, Profiles[1].HalfPower );
    EEPROM.update( 0x14, Profiles[1].ROF );
    EEPROM.update( 0x16, Profiles[1].BurstSize );
    EEPROM.update( 0x18, Profiles[1].MagSize );
    EEPROM.update( 0x1A, Profiles[1].FireOnEmptyMag );
    EEPROM.update( 0x1C, Profiles[1].AltAction );

    EEPROM.update( 0x20, Profiles[2].FullPower );
    EEPROM.update( 0x22, Profiles[2].HalfPower );
    EEPROM.update( 0x24, Profiles[2].ROF );
    EEPROM.update( 0x26, Profiles[2].BurstSize );
    EEPROM.update( 0x28, Profiles[2].MagSize );
    EEPROM.update( 0x2A, Profiles[2].FireOnEmptyMag );
    EEPROM.update( 0x2C, Profiles[2].AltAction );

    EEPROM.update( 0x30, Profiles[3].FullPower );
    EEPROM.update( 0x32, Profiles[3].HalfPower );
    EEPROM.update( 0x34, Profiles[3].ROF );
    EEPROM.update( 0x36, Profiles[3].BurstSize );
    EEPROM.update( 0x38, Profiles[3].MagSize );
    EEPROM.update( 0x3A, Profiles[3].FireOnEmptyMag );
    EEPROM.update( 0x3C, Profiles[3].AltAction );        

    EEPROM.update( 0x40, EStopOnPusherJam );
    EEPROM.update( 0x42, EStopOnDartJam );
    EEPROM.update( 0x44, CurrentProfile );
  }

  // Update the Lifetime Darts Fired. 32 bit value
  uint16 LDF1 = (uint16)LifetimeDartsFired;
  uint16 LDF2 = (uint16)(LifetimeDartsFired >> 16);
  EEPROM.update( 0x46, LDF1 );  
  EEPROM.update( 0x48, LDF2 );  
}
